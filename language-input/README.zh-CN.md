...
- AI 模式默认使用本机 QuickMT 三组件路线，支持英语、日语和西班牙语。M2M100 418M INT8 保留作兼容实验选项，但冷请求实测超过 1 秒，不推荐用于打字。当前模型与版本见 [`MODEL.zh-CN.md`](MODEL.zh-CN.md)。
...
AI 译注在候选窗中只显示释义正文，例如 `hello; hi`、`こんにちは`、`hola`；内部仍保留来源信息供朗读解析。英语词典译注默认带 `〔en·词〕` 标签；设置中的「简洁译注」可隐藏词典标签。
...
源码仍保留 OpenAI Chat Completions 兼容端点，供开发与对比测试使用；正式安装在存在本地 Host 时默认使用本地模型，不需要网络或 API key。在设置应用中选择外接 API 后，选择会保存并在服务重新启动时生效；显式环境变量 `LANGUAGE_INPUT_REMOTE_ENABLED=1` 也可覆盖已保存选择。可配置变量见源码中的 `LoadRemoteGlossConfig`；真实 key 不应写入源码、Rime 配置或截图。
