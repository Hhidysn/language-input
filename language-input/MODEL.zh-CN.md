...
当前版本的 AI 翻译采用 **QuickMT 三组件翻译路线**
`quickmt-gloss-route-v2`。M2M100 因冷请求超过 1 秒，已从输入法开关和设置页移除。核心安装包不携带任何模型权重；导入相应的
`.limodel` 包后才会在本机运行。
...
## 历史实验：M2M100 418M INT8

M2M100 曾作为与 QuickMT 分离的实验后端。当前输入法只选择 QuickMT；旧版
M2M100 包格式和 Host 支持保留供历史测试，不再从设置页安装或切换。
...
633 MiB、650 MiB、640 MiB。冷请求超过 1 秒，不满足当前打字体验要求；因此已从用户可选配置移除。合法输出率也不代表语义质量。
...
| **M2M100 418M INT8（已从配置移除）** | 418M；CTranslate2 INT8 包约 477 MiB | 101 语言多语种 encoder-decoder，MIT | 冷请求超过 1 秒，不满足输入法要求 |
...
QuickMT 是当前打字场景的本地后端，因为它的三条定向路线已有低延迟热请求基线。启用 AI 译注的普通输入框获得焦点时，输入法在后台预加载当前 QuickMT 语言路线，不发送候选文字；目标语言切换也会请求预加载。预加载完成后，首次候选页走已加载模型。若聚焦后立即打字、Host 刚退出或模型正在加载，首次请求仍可能超过 1 秒，热请求数据不能当成首次请求的保证。

## 与输入法开关的关系

- `翻译来源：AI`：使用当前目标语言的 QuickMT 路线。
...
和 Tencent Hy-MT2-1.8B 的历史筛选证据保留在
