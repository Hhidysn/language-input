...
633 MiB、650 MiB、640 MiB。这个结果说明它适合继续作为“可选尝试”使用，但冷启动仍会
明显慢于 QuickMT，且合法输出率不代表语义质量。
...
| **M2M100 418M INT8（当前可选）** | 418M；CTranslate2 INT8 包约 477 MiB | 101 语言多语种 encoder-decoder，MIT | 独立直达三语言路线；延迟更适合尝试，但语义质量仍需按盲测逐项验收 |
...
QuickMT 仍是默认后端，因为它的三条定向路线已有输入法场景的低延迟基线；M2M100
作为用户可选的独立后端，用同一 120 条集记录合法输出、安全拦截、候选页延迟和峰值内存，
不改变默认行为。两者的语义质量仍应按实际目标语言和词义逐项判断，合法输出率不等于翻译质量。
