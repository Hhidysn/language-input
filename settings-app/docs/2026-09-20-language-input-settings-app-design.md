...
- **页面**（rev8 更新：下表为当前实现；「本批」指 2026-09-22 的功能批）：

  | # | 页面 | 能力（★ = 本批新增 / 变更） |
  |---|---|---|
  | 1 | 概览 | 方案 / 译注开关 / 后端 / 模型状态（只读） |
  | 2 | 翻译 | 本地 ↔ 外接 API（按 §5.2 语义表）+ 端点/Key/模型/语言；★「测试端点连通性」：单次最小请求、8 s 超时、不重试，遵守 `allow_http`/代理策略，密钥仅入请求头且结果脱敏（`remote_probe.py`，经 D1 worker） |
  | 3 | 模型 | 包列表、状态与语言可用性；★表格行选择驱动的真实操作：**下载**（由下载进度回调驱动的确定性 `QProgressBar`）、**从目录安装**、**从模型包文件导入**（已验证 `.limodel`）、**校验**；★安装确认框先显示 `requires` 依赖（缺失时单独警告）；★「替换已安装组件」勾选（复用 backup/swap/rollback）；全部经 D1 worker 离线程执行 |
  | 4 | 词库与记忆 | 学习开关（★文案写明"关闭会同时停用学习与已学词条"）+ 同步 / 词典管理（**不含**词频列表，见 §8） |
  | 5 | 外观 | 候选窗配色 / 字号 / 横竖排 / 行内预编辑（写 `weasel.custom.yaml`） |
  | 6 | 按键与开关 | F4 隐藏开关 → 受约束的可视化配置（按 §5.7）；★去除 reset 补丁区只保留「方案」下拉这一个刷新触发器（删除重复的「刷新」按钮）；★新增「还原重置补丁」（GUI 回退，确认 + 空 stderr 判定） |
  | 7 | 关于 | 版本 / 设计文档入口；「重新部署」★增加确认且要求 **stderr 为空**（exit 0 不再视为成功）；★「恢复默认设置」（`restore_defaults.py`，逐项报告，不动已安装模型）；★「清除 AI 缓存」（`server.clear_ai_cache` 公共包装）；★「打开应用配置目录」 |
...
| 27 | **模型页真实操作**（2026-09-22 批）：下载 / 从目录安装 / 从模型包（`.limodel`）导入 / 校验全部走 D1 worker（离线程 + BusyStrip）；下载用 `ProgressBridge` 把 worker 线程的进度回调经信号汇总到主线程并驱动**确定性** `QProgressBar`（`theme.qss` 补 `QProgressBar` 规则，色值取自既有调色板 token）；安装确认先显示 `requires` 依赖（缺失时单独警告），支持「替换已安装组件」勾选；非空错误结果绝不判成功；本应用**不提供删除已安装组件**（安装层无安全移除接口，替换是受支持路径） | ✅ 已完成（`.work` stub 探针 **26/26**：每个新动作经真实 handler + stub 后端，断言离主线程 / 确定性进度条更新 / 结果入 UI；探针用后即删） |
| 28 | **维护动作补齐**（同批）：①「恢复默认设置」= `restore_defaults.py`，逐项还原（方案 reset 补丁 / 学习覆盖 / `weasel.custom.yaml` 托管样式键 / 简洁译注影子副本 / `user.yaml` 选项，后者在 server 停止窗口内编辑），末尾一次 `/deploy`（成功 = exit 0 且 stderr 空），**绝不触碰模型根**，无法安全还原（流式 `option:` 映射）按项报告；②「清除 AI 缓存」= `server.clear_ai_cache` 公共包装（先确认）；③「打开应用配置目录」；④「重新部署」加确认且 **exit 0 + stderr 空** 才算成功；⑤按键页 reset 区去掉与方案下拉重复的「刷新」按钮（下拉即唯一刷新触发器）；⑥开关标签统一以 `rime_settings.SWITCH_LABELS` 为唯一来源（删除 app.py 的重复定义）；⑦`yaml_io.remove_user_yaml_option` 定点删除 `var/option/<name>`；⑧学习开关可见文案与确认均写明「关闭会同时停用学习与已学词条的使用」 | ✅ 已完成（同一 26/26 探针覆盖；设计系统复核 A1/A2/A3/A6 相对 git HEAD 基线 **6/6 通过**：无新增偏离色板颜色 / 间距刻度 / 裸标识符标签） |
